...
This is a root-level handler that can be used with handler path: handler.main
"""
...
from src.pc_analyzer import PCAnalyzer
from src.alerter import Alerter, AlertConfig
...
def main(event: dict, context) -> dict:
    ...
    if not api_token:
        return {
            "statusCode": 401,
            "body": {
                "error": "Missing SAMSARA_API_KEY environment variable."
            }
        }
    ...
    try:
        ...
        body = event.get("body", {})
        if isinstance(body, dict) and body.get("include_all_pc_drivers"):
            response_body["all_pc_drivers"] = [r.to_dict() for r in drivers_in_pc]
